...
contract MixinMakerPSM {
    ...
    function _tradeMakerPsm(
        ...
        if (address(sellToken) == data.gemTokenAddres) {
            ...
            uint256 feeDivisor = WAD.add(psm.tout()); // eg. 1.001 * 10 ** 18 with 0.1% fee [tout is in wad];
            uint256 buyTokenBaseUnit = uint256(10) **
                uint256(ERC20(address(buyToken)).decimals());
            uint256 gemAmount = sellAmount.mul(buyTokenBaseUnit).div(
                feeDivisor
            );
